        ...
        <el-main>
            <el-form :inline="true" :model="formData" label-position="top">
                ...
                <el-form-item label="描述">
                    <el-input v-model="formData.new_note" placeholder="" clearable />
                </el-form-item>
            ...
            <el-table ref="tableRef" :data="tableData" style="width: 100%" @row-dblclick="dblclick">
                ...
                <el-table-column prop="new_file_type" label="文件类型" width="180" />
                <el-table-column prop="new_createdon" label="创建时间" width="180" />
                <el-table-column prop="new_modifiedon" label="修改时间" width="180" />
                <el-table-column align="right">
                    <template #header>
                        <el-button size="small" :icon="Plus" type="success" circle
                        @click="createLine"></el-button>
                        <el-button size="small" :icon="RefreshRight" type="success" circle
                        @click="refreshLine"></el-button>
                    </template>
...
function loadData() {
    if (id.value) {
        req.get(`/api/new_template_group/get/?id=${id.value}`).then(res => {
...
/**保存 */
function save() {
    req.post("/api/new_template_group/save/", formData.value).then(res => {
        console.log("res", res);
        id.value = res.data;
        loadData();
    }).catch(err => {
